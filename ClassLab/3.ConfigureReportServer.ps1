...
$ComputerName = "$($labSettings.labPrefix)SQL"
...
if ($startingMachineNumber -eq 1) {
    Invoke-LabCommand -ComputerName $computerName -ActivityName 'Configure Report Server' -ArgumentList $settings.sqlUser, $settings.sqlPassword -ScriptBlock {
        ...
        If (!$configset.IsInitialized) {
            ...
            $conn = New-Object Microsoft.SqlServer.Management.Common.ServerConnection -ArgumentList "$env:ComputerName\ONLINE"
            ...
            $configset.SetDatabaseConnection("$env:ComputerName\ONLINE", "ReportServer", 1, $sqlUser, $sqlPassword)
