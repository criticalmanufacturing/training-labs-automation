...
Invoke-LabCommand -ComputerName SNAPPSRV01 -ActivityName 'Prepare SQL Server Distribution Media' -ArgumentList "$($labPrefix)DC1", "$($labPrefix)DC1.($domain)", $domain, $adminCredential -ScriptBlock { 
param
(
    [string]$dhcpDnsName,
    [string]$dcDnsName,
...
Set-DhcpServerv4OptionValue -OptionID 3 -Value 192.168.80.3 -ScopeID 192.168.80.0 -ComputerName $dhcpDnsName
